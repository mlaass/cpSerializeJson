...
#include "json.hpp"
static void cpSerializeJsonSetBodySpace(cpSpace* space);
static void cpSerializeJsonClearBodyConstraintMap();
static void cpSerializeJsonSetShapeBody(cpBody* body);
static void cpSerializeJsonSetShapeSpace(cpSpace* space);
...
template <> struct adl_serializer<struct cpBody*> {
  ...
  static cpBody* from_json(const json& j) {
    ...
    if (j.is_null()) {
      ...
      cpSerializeJsonSetShapeBody(body);
      cpSerializeJsonSetShapeSpace(space);
      ...
      cpSerializeJsonSetShapeBody(nullptr);
      cpSerializeJsonSetShapeSpace(nullptr);
    }
    return body;
  }
};
...
template <> struct adl_serializer<cpConstraint*> {
  ...
  static cpConstraint* from_json(const json& j) {
    ...
    if ("GearJoint" == j["type"]) {
      ...
      ct = cpDampedSpringNew(
          a, b, anchorA, anchorB, restLength, stiffness, damping);
...
static nlohmann::json cpSerializeJsonBodySerializeConstraints(cpSpace* space) {
  nlohmann::adl_serializer<struct cpConstraint*>::space = space;
...
static void cpSerializeJsonBodyDeserializeConstraints(
    cpSpace* space, const nlohmann::json& contraints) {
  nlohmann::adl_serializer<struct cpConstraint*>::space = space;
  nlohmann::adl_serializer<struct cpConstraint*>::bodyMap =
      &nlohmann::adl_serializer<struct cpBody*>::bodyMap;
